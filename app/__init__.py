...
def load_into_dict_sqlite():
    ...
    c = conn.cursor()
    rs = c.execute(
        """SELECT word, pos, ipa, definition
                      FROM words
                      ORDER BY word"""
    )
    for r in rs:
        ...
        res[r["word"]]["definitions"].append(
            {"pos": r["pos"], "definition": r["definition"]}
        )
    end = time.time()
    conn.close()
    print(
        "Successfully loaded {} word definitions from SQLite DB into Dict in: {}".format(
            len(res), end - start
        )
    )
    return res
...
def load_thesaurus_into_dict_sqlite():
    ...
    c = conn.cursor()
    rs = c.execute(
        """SELECT word, pos, origin, thesaurus
                      FROM words
                      ORDER BY word"""
    )
    for r in rs:
        ...
        res[r["word"]]["thesaurus"].append(
            {"pos": r["pos"], "origin": r["origin"], "thesaurus": r["thesaurus"]}
        )
    end = time.time()
    conn.close()
    print(
        "Successfully loaded {} thesaurus definitions from SQLite DB into Dict in: {}".format(
            len(res), end - start
        )
    )
    return res
