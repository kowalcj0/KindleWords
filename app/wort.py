    ...
    with open(filename, "r") as infile:
        ...
        for line in clippings:
            if len(line.split()) <= 2:
                ...
                if no_specials:
                    no_dots = no_specials.replace(".", "")
                    no_eses = no_dots.replace("’s", "")
...
def define_wordnet(words):
    conn = sqlite3.connect("sqlite-31_snapshot.db")
    c = conn.cursor()
    to_translate = words[-10:]
    for word in to_translate:
        t = (word,)
        query = "SELECT lemma, definition " "FROM wordsXsensesXsynsets " "WHERE lemma=?"
...
def define_dictionarycom(words):
    ...
    for word in to_translate:
        t = (word,)
        query = (
            "SELECT e.entry, e.pronunciation_ipa, cb.content "
            "FROM entries e "
...
if __name__ == "__main__":
    ...
    define_dictionarycom(words)
    define_wordnet(words)
